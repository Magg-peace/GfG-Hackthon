...
export default function Home() {
  ...
  const handleSendMessage = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        // Use follow-up if we have prior context and it looks like a refinement
        const isFollowUp =
          lastQuery &&
          lastSql &&
          sessionId &&
  ...
  const handleUploadComplete = useCallback(
    (result: UploadResponse) => {
      ...
      // Get new suggestions for uploaded data
      getSuggestions(result.session_id).then((res) =>
        setSuggestions(res.suggestions)
      );
      ...
      addMessage(
        "assistant",
        `Successfully loaded **${result.filename}** (${result.row_count.toLocaleString()} rows, ${result.columns.length} columns: ${result.columns.join(", ")}). You can now ask questions about this data!`
      );
    },
    [addMessage]
  );
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto bg-slate-950/50">
          ...
          {/* Charts Dashboard */}
          <Dashboard charts={charts} summary={summary} />
        </div>
      </div>
    </div>
  );
