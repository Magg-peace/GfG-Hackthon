...
function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div className="bg-[#0c1021]/95 backdrop-blur-xl border border-[#1c2340] rounded-xl px-4 py-3 shadow-xl">
    ...
    <div>
      <div className="overflow-x-auto rounded-lg border border-[#1c2340]">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#1c2340] bg-[#080c18]">
              {columns.map((col) => (
                <th
                  key={col}
          ...
          <tbody>
            {paginated.map((row, i) => (
              <tr
                key={i}
                className="border-b border-[#151b30] hover:bg-[#0e1225] transition-colors"
              >
                {columns.map((col) => (
        ...
        <div className="flex items-center justify-between mt-3 text-[11px] text-[#5a6380]">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1 rounded-lg bg-[#0c1021] border border-[#1c2340] hover:bg-[#151b30] hover:border-[#4f8fff]/20 disabled:opacity-30 transition-all"
            >
              Prev
            </button>
            <button
  ...
  if (chart.error) {
    return (
      <div className="bg-[#0e1225] border border-[#1c2340] rounded-2xl p-6 animate-slide-up">
        ...
        <div className="flex items-center gap-3 text-[#f87171] bg-[#f87171]/10 rounded-xl px-4 py-3 border border-[#f87171]/20">
        ...
        {chart.sql_executed && (
          <details className="mt-3">
            ...
            <pre className="mt-2 text-xs bg-[#080c18] rounded-xl p-4 overflow-x-auto text-[#8b95b0] border border-[#1c2340] font-mono">
              {chart.sql_executed}
            </pre>
          </details>
        )}
  ...
  return (
    <div className="glass-card-hover gradient-border p-6 animate-slide-up">
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0 ml-3">
          {chart.row_count !== undefined && (
            <span className="text-[10px] text-[#5a6380] bg-[#080c18] px-2 py-1 rounded-md border border-[#1c2340]">
          ...
          <button
            ...
            className="text-[10px] text-[#5a6380] hover:text-[#8b95b0] bg-[#080c18] px-2 py-1 rounded-md border border-[#1c2340] transition-all duration-200 hover:border-[#4f8fff]/30"
          >
      ...
      {showSql && chart.sql_executed && (
        <pre className="mb-4 text-[11px] bg-[#080c18] rounded-xl p-4 overflow-x-auto text-[#34d399]/70 border border-[#1c2340] font-mono leading-relaxed">
          {chart.sql_executed}
        </pre>
      )}

      {/* Chart */}
      <div className="mt-2 pt-4 border-t border-[#1c2340]">
        {renderChart()}
      </div>
    </div>
  );
